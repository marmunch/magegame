document.addEventListener('DOMContentLoaded', function() {
    ...
    function startLongPolling() {
        fetch(`server/poll.php?room_id=${room_id}`)
            .then(response => response.json())
            .then(data => {
                handleServerMessage(data);
                setTimeout(startLongPolling, 4000); // Отправляем новый запрос каждые 4 секунды
            })
            .catch(error => {
                console.error('Ошибка long polling:', error);
                setTimeout(startLongPolling, 4000); // Повторяем попытку через 4 секунды в случае ошибки
            });
    }
    ...
    function fetchCurrentTime() {
        fetch(`server/get_current_timee.php?room_id=${room_id}`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
    ...
    function updateCurrentTime() {
        fetch(`server/update_current_timee.php`, {
            method: 'POST',
            headers: {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (allPlayersReady && allCardsSelected && phase === 1) {
                        phase = 2;
                        console.log('Sending startPhase2 message via Long Polling');
                        fetch('server/start_phase2.php', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded',
                            },
    ...
    function checkPhase2(room_id) {
        fetch(`server/get_game_info.php?room_id=${room_id}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Ошибка сети: ' + response.statusText);
            }
    ...
    function startPhase2() {
        ...
        .then(() => {
            fetch(`server/get_chosen_cards.php?room_id=${room_id}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Ошибка сети: ' + response.statusText);
                }
        ...
        if (cardQueue.length > 0) {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    fetch(`server/remove_spells.php`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
            ...
            fetch(`server/check_spells_empty.php?room_id=${room_id}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Ошибка сети: ' + response.statusText);
                }
    ...
    function checkWinner() {
        fetch(`server/check_winner.php?room_id=${room_id}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Ошибка сети: ' + response.statusText);
            }
    ...
    function applyCardEffects(card, myLogin) {
        ...
        fetch('server/apply_card_effects.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
    ...
    function updateCardsInHand() {
        fetch(`server/update_cards_in_hand.php?room_id=${room_id}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Ошибка сети: ' + response.statusText);
            }
    ...
    function resetCardsChosen() {
        fetch(`server/reset_cards_chosen.php?room_id=${room_id}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Ошибка сети: ' + response.statusText);
            }
